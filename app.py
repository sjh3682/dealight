...
# 깃허브에 파일이 폴더 없이 평평하게 올라가 있어도 작동하도록,
# 템플릿(html)과 정적파일(css)을 현재 폴더에서 찾게 한다.
app = Flask(__name__, template_folder=".", static_folder=".", static_url_path="/static")

# listings.csv 도 app.py 와 같은 폴더에서 찾는다.
DATA_PATH = os.path.join(os.path.dirname(__file__), "listings.csv")
...
if __name__ == "__main__":
    # Render 같은 배포 환경은 PORT 환경변수로 포트를 정해준다.
    # 로컬에서 실행하면 PORT가 없으므로 기본 5000번을 쓴다.
    ...
    # host=0.0.0.0 이어야 외부(배포 서버/다른 기기)에서 접속할 수 있다.
    app.run(host="0.0.0.0", port=port, debug=not on_server)
